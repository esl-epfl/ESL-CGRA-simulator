...
def load_operation_characterization(
    characterization_type, mapping_file="operation_characterization.csv"
):
    operation_mapping = {}
    csv_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), mapping_file
    )
...
def select_power_factors(characterization_factors):
    ...
    if "passive_power_w" in characterization_factors:
        operation_passive_power_mapping = load_operation_characterization(
            "passive_power_w"
        )
    ...
    if "reconfig_power_w" in characterization_factors:
        operation_reconfig_power_mapping = load_operation_characterization(
            "reconfig_power_w"
        )
...
(
    ...
    operation_clk_gate_mapping,
    operation_reconfig_power_mapping,
) = select_power_factors(characterization_factors)
...
(operation_latency_mapping, bus_type_active_row_coef, bus_type_cpu_loop_instrs) = (
    select_latency_factors(characterization_factors)
)
...
def get_latency_cc(cgra):
    ...
    cgra.max_latency_instr.latency_cc = max(
        longest_alu_op_latency_cc, total_mem_latency_cc
    )
...
def get_latency_alu_cc(cgra):
    for r in range(cgra.N_ROWS):
        for c in range(cgra.N_COLS):
            cgra.cells[r][c].latency_cc = int(
                operation_latency_mapping[cgra.cells[r][c].op]
            )
            if (
                cgra.max_latency_instr is None
                or cgra.cells[r][c].latency_cc > cgra.max_latency_instr.latency_cc
            ):
...
def get_latency_mem_cc(cgra):
    ...
    if any(
        elem in characterization_factors for elem in ("uniform_op_cc", "dma_per_cell")
    ):
        return 1
    return latency_cc
...
def compute_bank_index(cgra, r, c):
    if cgra.memory:
        base_addr = (
            cgra.init_store[0]
            if cgra.cells[r][c].op == "SWD"
            else sorted(cgra.memory)[0][0]
        )
    if cgra.memory_manager.bus_type == "INTERLEAVED":
        index_pos = int(
            ((cgra.cells[r][c].addr - base_addr) / cgra.memory_manager.word_size_B)
            % cgra.memory_manager.banks_n
        )
...
def group_dma_accesses(cgra):
    ...
    for r in range(cgra.N_ROWS):
        for c in range(cgra.N_COLS):
            if (
                cgra.cells[r][c].op in OPERATIONS_MEMORY_ACCESS
                and (r, c) not in cgra.covered_accesses
            ):
                cgra.covered_accesses, concurrent_accesses = mark_access(
                    cgra.covered_accesses,
                    concurrent_accesses,
                    r,
                    c,
                    r,
                    cgra.cells[r][c].bank_index,
                )
            else:
                for k in range(cgra.N_ROWS):
                    if (
                        cgra.cells[k][c].op in OPERATIONS_MEMORY_ACCESS
                        and (k, c) not in cgra.covered_accesses
                    ):
                        cgra.covered_accesses, concurrent_accesses = mark_access(
                            cgra.covered_accesses,
                            concurrent_accesses,
...
def rearrange_accesses(cgra, concurrent_accesses):
    if cgra.memory_manager.bus_type == "INTERLEAVED":
        for i in range(cgra.N_ROWS - 1, 0, -1):
            order_pairs = [
                pair[1] for pairs in concurrent_accesses[i].values() for pair in pairs
            ][::-1]
            concurrent_accesses[i - 1] = {
                key: sorted(
                    pairs,
                    key=lambda x: (
                        order_pairs.index(x[1]) if x[1] in order_pairs else float("inf")
                    ),
                )
    ...
    for i in range(cgra.N_ROWS):
        for values in cgra.concurrent_accesses[i].values():
            for current_access in values:
                ...
                current_pos = (
                    find_position(cgra.concurrent_accesses[i], current_access[1]) + 1
                )
                next_pos = (
                    find_position(cgra.concurrent_accesses[i + 1], current_access[1])
                    if i < cgra.N_ROWS - 1
                    else 0
                )
...
def get_power_w(cgra):
    ...
    for r in range(cgra.N_ROWS):
        for c in range(cgra.N_COLS):
            ...
            get_cell_energy_j(
                cgra.cells[r][c],
                cgra.max_latency_instr.latency_cc,
                cgra.reconfig_consumption_w[r][c],
            )
            cgra.energy[-1][r][c] = cgra.cells[r][c].energy
            cgra.power[-1][r][c] = cgra.cells[r][c].energy / (
                cgra.max_latency_instr.latency_cc * CLK_PERIOD
            )
    if cgra.exit:
...
def fetch_operation_value(cell, type, op=None, latency=None):
    def get_value(mapping, op_key, latency):
        if SCALE_FACTOR:
            ...
            if latency > max(mapping[op_key]) and type == "power":
                return mapping[op_key][max(mapping[op_key])]
            return mapping[op_key].get(latency, mapping[op_key].get(cell.latency_cc, 0))

    if type == "power":
        op_key = (
            f"{op}_{cell.params_info}"
            if (f"{op}_{cell.params_info}") in operation_power_mapping
            else op
        )
...
def get_cell_reconfig_w(cgra, r, c):
    ...
    key = (
        f"{current_reconfig[1]}-{current_reconfig[1]}"
        if current_reconfig[0] == ""
        else f"{current_reconfig[0]}-{current_reconfig[1]}"
    )
...
def get_cell_energy_j(cell, latency, reconfig_consumption_w):
    ...
    cell.energy += reconfig_consumption_w
    cell.energy *= CLK_PERIOD
    ...
    for index, item in enumerate(cgra.instr_latency_cc):
        for row_idx in range(cgra.N_ROWS):
            for col_idx in range(cgra.N_COLS):
                cgra.energy_array[row_idx][col_idx] += cgra.energy[index][row_idx][
                    col_idx
                ]
    cgra.avg_energy = sum([energy for row in cgra.energy_array for energy in row])
    # Adjust energy to remove dead time consumption
    cgra.avg_energy -= DEAD_TIME_CONSUMPTION_W * DEAD_TIME_LATENCY_CC * CLK_PERIOD
...
def display_characterization(cgra, pr):
    ...
    if any(item in pr for item in ["OP_MAX_LAT", "ALL_LAT_INFO"]):
        ...
        for index, item in enumerate(cgra.instr_latency_cc):
            print(
                "{:<2} {:<6} {:<25} {:<10}".format(
                    index + 1, f"({item.instr2exec})", item.instr, item.latency_cc
                )
            )
    ...
    if any(
        item in pr
        for item in ["AVG_INSTR_PWR_INFO", "ALL_PWR_EN_INFO", "FINAL_EN_INFO"]
    ):
        print(
            "\nPower estimation for all instructions:",
            format((cgra.avg_energy) / (cgra.total_latency_cc * CLK_PERIOD), ".2e"),
            " W",
        )
    if any(
        item in pr for item in ["AVG_INSTR_EN_INFO", "ALL_PWR_EN_INFO", "FINAL_EN_INFO"]
    ):
        print("\nAverage power per operation:\n")
        the_sum = 0
        for row_idx in range(cgra.N_ROWS):
            line = cgra.energy_array[row_idx]
            out_string = " ".join(
                f"{elem / (cgra.total_latency_cc * CLK_PERIOD):.2e}" for elem in line
            )
            for elem in line:
                the_sum += elem / (cgra.total_latency_cc * CLK_PERIOD)
            print(out_string)
        print("\nClock period used:", CLK_PERIOD, "s")
        print(
            "\nTotal energy consumed during execution:",
            format(cgra.avg_energy, ".2e"),
            "J",
        )
